...
// Inject the view mode state and logic
...
window.renderFinanceTable = function() {
  ...
  if (currentFinView === 'summary') {
    ...
    tbody.innerHTML = summaryGroups.map(g => {
      ...
      return `
        <tr>
          <td style="font-weight: bold;">\${escapeHtml(g.label)}</td>
          <td>\${g.count}</td>
          <td>\${formatMoney(g.allocated, 'USD')}</td>
          <td>\${formatMoney(g.expenses, 'USD')}</td>
          <td>\${formatMoney(g.returned, 'USD')}</td>
          <td><span class="status-pill \${statusClass}">\${formatMoney(net, 'USD')}</span></td>
        </tr>
      `;
    }).join('');
