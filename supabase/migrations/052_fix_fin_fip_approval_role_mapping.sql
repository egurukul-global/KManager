-- Migration 052: Fix FIN and FIP approval role mapping in user_has_approval_role and users_with_approval_role
...
BEGIN
  ...
  IF v_role = 'FIN' AND v_org_role IN ('fin', 'admin') THEN
    RETURN true;
  END IF;

  IF v_role = 'FIP' AND v_org_role IN ('fip', 'admin') THEN
    RETURN true;
  END IF;
...
BEGIN
  ...
  FROM (
    ...
    WHERE
      ...
      OR (v_role = 'FIN' AND lower(coalesce(u.role, '')) = 'fin')
      OR (v_role = 'FIP' AND lower(coalesce(u.role, '')) = 'fip')
