...
export function getTransferFundsPage() {
  ...
  const lead = isTeamLeadAccess(state);
    ...
    return `
      ...
      <p class="page-intro">
        Send and receive money across the organization. This engine handles Org-to-Team, Team-to-Org (Unused Funds), and internal team transfers. All transfers require a two-part handshake (Send & Accept).
      </p>
...
async function executeFundsTransfer(e) {
  ...
  if (flow.status === TRANSFER_STATUS.ACCEPTED) {
    if (srcBucket._can_view_balance !== false && (parseFloat(srcBucket.balance) || 0) < amount) {
  ...
  const transferPayload = {
    ...
    linked_budget_id: crossTeam ? crossBudgetId : (document.getElementById('trLinkedBudgetId')?.value || null),
